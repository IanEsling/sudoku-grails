...
class TestCell {
    ...
    void cellReport() {
        ...
        assertEquals("wrong report from cell", "1,2 cannot be a 1, 2 or 3 because they are in test unit 1",
                testee.report[0].toString())
        unit = [solvedNumbers: {Sets.newHashSet(8)}, toString: {"test unit 2"}] as Unit
        testee.remove unit
        assertEquals("wrong report[0] from cell", "1,2 cannot be a 1, 2 or 3 because they are in test unit 1",
                testee.report[0].toString())
        assertEquals("wrong report[1] from cell", "1,2 cannot be a 8 because it is in test unit 2",
        ...
        assertEquals("wrong report[1] from cell", "1,2 cannot be a 8 because it is in test unit 2",
                testee.report[1].toString())
        assertEquals("wrong report[2] from cell", "1,2 cannot be a 9 because it is in test unit 3",
